...
function Explore() {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      try {
        const response = await fetch("http://localhost/skillswap/backend/fetch_notes.php");
        const data = await response.json();

        if (data.success) {
          setNotes(data.data);  // data.data is already grouped by category
        } else {
      ...
      <div className="flex items-center justify-center h-[30rem] w-full bg-blue-500 gap-20 shadow-md">
        <div>
          ...
          <p className="max-w-[30rem] text-xl text-white">
            From critical skills to technical topics, Skillswap supports your professional development.
          </p>
        </div>
        <img src="note-illustration-2.png" className="h-[27rem] w-auto" alt="Illustration" />
      </div>
                    ...
                    >
                      <Link to={`/note/${note.note_id}`}>
                        ...
                        <div className="pl-3">
                          ...
                          <p className="text-sm text-gray-600 mb-2">{note.description || "No description"}</p>
                        </div>
                      </Link>
                    </div>
                  ))}
